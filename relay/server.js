...
const { getAccessToken, getGarage61TrackId } = require('./garage61.js');

app.get('/api/garage61/drivers/:id/stint-stats', requireAuth, async (req, res) => {
    const accessToken = getAccessToken();

    const garage61TrackId = await getGarage61TrackId(req.query.trackId);
    if (garage61TrackId == null) return res.status(404).json({ error: 'Track not found in Garage61' });

    const params = new URLSearchParams({ drivers: 'me', tracks: String(garage61TrackId), group: 'none', limit: '200' });
    ...
    if (cleanLaps.length === 0) return res.json({ avgFuelPerLap: null, bestLapTime: null, sampleSize: 0 });
